...
/**
 * The class that builds the simplified plain (not compressed) statement-level
 * index.
 *
 * @author Zdenek Tronicek, tronicek@tarleton.edu
 */
public class SimplifiedIndexStmtBuilderNaive extends IndexBuilder {
    ...
    private final Deque<Range> methods = new ArrayDeque<>();

    public SimplifiedIndexStmtBuilderNaive(Properties conf, Path srcDir) {
        super(conf, srcDir);
        stack = new Stack();
    }
    ...
    private void renameAppendId(String id) {
        for (StackNode node : stack) {
            RenameStrategy rs = node.getRenameStrategy();
            ...
            List<String> labs = node.getLabels();
            labs.add("id:" + rid);
//            System.out.printf("-- renamed: %s -> %s%n", id, rid);
        }
    }
    ...
    private Pos pos(Node n) {
        ...
        Range mr = methods.peek();
        return new Pos(project, srcFile, begin, end, mr.begin, mr.end);
    }
    ...
    public void visit(ClassOrInterfaceDeclaration n, Void arg) {
        ...
        n.getAnnotations().forEach(p -> p.accept(this, arg));
        n.getModifiers().forEach(p -> p.accept(this, arg));
        renameAppendId(n.getNameAsString());
        enterBlock();
    ...
    public void visit(ConstructorDeclaration n, Void arg) {
        inMethod++;
        methods.push(n.getRange().orElse(null));
        append(n);
        n.getAnnotations().forEach(p -> p.accept(this, arg));
        n.getModifiers().forEach(p -> p.accept(this, arg));
        if (n.getTypeParameters().isNonEmpty()) {
    ...
    public void visit(Name n, Void arg) {
        ...
        String ident = n.getIdentifier();
        if (isVar(ident)) {
            renameAppendId(ident);
        } else {
            appendId(ident);
        }
        appendEnd(n);
    ...
    private boolean isVar(String ident) {
